...
export class MyStack extends TerraformStack {
  constructor(scope: Construct, id: string, props: MyStackProps) {
    ...
    // All NiFi sensitive material lives in Secrets Manager, owned by the
    // dedicated ebs_test_secrets stack (deployed FIRST; populated via
    //   aws secretsmanager put-secret-value --secret-id <name> --secret-string <value>
    // — the values survive cluster teardown). This stack only LOOKS the
    // seven secrets UP by their deterministic names: the data sources fail
    // at plan time until the secrets stack is deployed, which is the
    // intended ordering guard between the otherwise-independent stacks
    // (see README-nifi-cluster.md).
    const nifiSecrets: NifiSecretRefs = new NifiSecretsLookup(this, "nifi-secrets", {
      clusterName,
    });
    ...
    const slotServices = perSlot(
      (slot) =>
        new SlotService(this, `svc-${slot.name}`, {
          ...
          namespaceName,
          sensitiveKeySecretArn: nifiSecrets.sensitivePropsKeyArn,
          tlsKeystorePasswordArn: nifiSecrets.tlsKeystorePasswordArn,
          tlsTruststorePasswordArn: nifiSecrets.tlsTruststorePasswordArn,
          tlsKeystoreB64Arns: nifiSecrets.tlsKeystoreB64Arns,
          tlsTruststoreB64Arn: nifiSecrets.tlsTruststoreB64Arn,
          tags: commonTags,
    ...
    const uiPortForwardHint =
      ...
      `--parameters "host=<NIFI_TASK_IP>,portNumber=8443,localPortNumber=8443" ` +
      `# import scripts/tls/out/clients/admin/admin.p12 into the browser first ` +
      `(cert-based auth), then browse https://localhost:8443/nifi (see README-nifi-cluster.md)`;
