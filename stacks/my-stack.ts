...
export class MyStack extends TerraformStack {
  constructor(scope: Construct, id: string, props: MyStackProps) {
    ...
    // All NiFi sensitive material lives in Secrets Manager as
    // placeholder-seeded shells — Terraform never manages the real values:
    // the sensitive-props key, the TLS keystore/truststore passwords, the
    // three per-node keystore.p12 blobs and the shared truststore.p12 (both
    // as base64 text; generated by scripts/tls). NiFi runs AUTH=tls from
    // first boot and CRASH-LOOPS (by design) until the four TLS material
    // secrets are populated:
    //   aws secretsmanager put-secret-value --secret-id <name> --secret-string <value>
    // then force a new deployment on the three nifi services (see
